...
{
    public partial class Audio_Delete : UserControl
    {
        public Audio_Delete()
        {
            InitializeComponent();
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    con.Open();
                    string deleteAudio = "DELETE FROM Audio WHERE Uri = @uri";
                    using (SqlCommand cmd = new SqlCommand(deleteAudio, con))
                    {
                        cmd.Parameters.AddWithValue("@uri", textBox16.Text);
                        int rowsAffected = cmd.ExecuteNonQuery();
                        if (rowsAffected > 0)
                        {
                            MessageBox.Show("Audio records successfully deleted!");
                            Home hm = new Home();
                            MainControlClass.showControl(hm, panel2);
                        }
                        else
                        {
                            MessageBox.Show("Failed to delete Audio records.");
                        }
                    }
                }
            }
