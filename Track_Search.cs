...
{
    public partial class Track_Search : UserControl
    {
        public Track_Search()
        {
            InitializeComponent();
        ...
        private void SearchButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    string search_track = "SELECT * FROM TRACK where " + Column + " " + condition + " @Value";
                    using (SqlCommand cmd = new SqlCommand(search_track, con))
                    {
